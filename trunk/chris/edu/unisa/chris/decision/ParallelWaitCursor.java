 ...
 */
public class ParallelWaitCursor extends Change {
    Goals goals;
    List initial;
    ...
    public ParallelWaitCursor(Goals goalsBeliefSet, List initialGoalInfos, boolean test) {
    ...
    public boolean condition(){
        ...
        try{
            int goalsSucceeded = 0;
            for(int i=0; i<initial.size(); i++){
                initInfo = (GoalInfo) initial.get(i);
                finInfo = (GoalInfo) goals.getGoalInfo(initInfo.name,initInfo.parent);
                if(finInfo.failureCount > initInfo.failureCount) //check if this subgoal has failed
                    return true;
                if(finInfo.successCount > initInfo.successCount) //check if this subgoal has succeeded
                    goalsSucceeded++;  //add 1 to the number of goals succeeded
                if(goalsSucceeded == initial.size()) //
                    return true;
            }
        }
