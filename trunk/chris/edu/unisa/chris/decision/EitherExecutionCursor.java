 ...
 */
public class EitherExecutionCursor extends Change {
    String grandParent;
    Goals goals;
    List initial;
    ...
    public EitherExecutionCursor(Goals goalsBeliefSet, String grandParentName, List initialGoalInfos, boolean test) {
    ...
    public boolean condition(){
        try{
            ...
            //now check subgoal for EITHER execution            
            for(int i=0; i<initial.size(); i++){
                initInfo = (GoalInfo) initial.get(i);
                finInfo = (GoalInfo) goals.getGoalInfo(initInfo.name,initInfo.parent);
                if(finInfo.failureCount > initInfo.failureCount || finInfo.successCount > initInfo.successCount)
                    return false;
            }
        }
