...
public class TenChoiceTestbed implements Environment{
    ...
    int optimalChoice;
    Random uniform = new Random(84573542);
    Random gaussian = new Random(13445364);
    
    double rewards[],rewardsTotal[];
    int choiceHistory[];
    double percentOptimal[],optimal[];
    int play = 0;
    ...
    public void step(int choice){
        ...
        if(play==0)
            rewardsTotal[play] = currentReward;
        else
            rewardsTotal[play] = rewardsTotal[play-1] + currentReward;
    ...
    public String getPlots(){
        ...
        sb.append("Play\tRewards\tRTotal\tOpt\t%Opt"+System.getProperty("line.separator"));
        for(int i=0; i<numPlays; i++){
            if((i+1)%100 == 0)
                sb.append((i+1) + "\t" + (((double)((int)(rewards[i]*100)))/100)
                                + "\t" +  (((double)((int)(rewardsTotal[i]*100)))/100)
                                + "\t" + optimal[i]
                                + "\t" + (((double)((int)(percentOptimal[i]*100)))/100)
